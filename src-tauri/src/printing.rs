...
use uuid::Uuid;
use windows_sys::Win32::Graphics::{Printing::PRINTER_HANDLE};
...
unsafe fn print_image(printer_name: &str, img: &DynamicImage) -> Result<(), String> {
  ...
  let page_w = GetDeviceCaps(hdc, HORZRES.try_into().unwrap());
  let page_h = GetDeviceCaps(hdc, VERTRES.try_into().unwrap());
  ...
  let image_is_landscape = src_w > src_h;
  let page_is_landscape = page_w > page_h;

  let (fit_w, fit_h) = if image_is_landscape != page_is_landscape {
    (page_h, page_w)
  } else {
    (page_w, page_h)
  };

  let scale_x = fit_w as f64 / src_w as f64;
  let scale_y = fit_h as f64 / src_h as f64;
  let scale = scale_x.min(scale_y);

  let dest_w = (src_w as f64 * scale).round() as i32;
  let dest_h = (src_h as f64 * scale).round() as i32;

  let x = (page_w - dest_w) / 2;
  let y = (page_h - dest_h) / 2;
...
pub async fn print_shipping_label(image_data: String) -> Result<(), String> {
  let res = tauri::async_runtime::spawn_blocking(move || {
    ...
    unsafe {
      print_image(&printer, &rotated_img)?;
    }
