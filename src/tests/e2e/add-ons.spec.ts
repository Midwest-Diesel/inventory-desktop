...
async function newAddon(page: Page, part: string, qtyValue: number) {
  ...
  await expect(qty).toHaveValue(String(qtyValue));

  await partNum.fill(part);
  await partNum.press('Tab');

  await expect(desc).toHaveValue('INJECTOR C9');
...
test.describe('Create addon and add to inventory', () => {
  ...
  test('Autofill stock number', async ({ page }) => {
    ...
    await expect(stock).toHaveValue('INJ7259');
  });
  ...
  test('Autofill stock number with engine number 1', async ({ page }) => {
    ...
    await expect(stock).toHaveValue(formatDateStockNum('INJ', 'A'));
  });
  ...
  test.skip('Send to office addons', async ({ page }) => {
    await newAddon(page, '4700', 1);
    const engineNum = page.getByTestId('engine-num').first();
    const stockNum = page.getByTestId('stock-num').first();
    ...
    const officeStock = page.getByTestId('stock-num').first();

    await expect(officeStock).toBeVisible();
    await expect(officeStock).toHaveValue(stockNumValue);
